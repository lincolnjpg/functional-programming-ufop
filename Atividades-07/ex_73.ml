...
let rec acertar secreto numX numY numTenta =
  ...
  if palpite = secreto then
    print_endline ("Parabens!!! Vc acertou e precisou de " ^ string_of_int numTenta ^ " tentativas!")
  else
    begin
      print_endline (if palpite > secreto then
                       ...
                       "Muito pequeno.");
      print_endline ("Tente novamente..." ^ "Ja foram realizadas " ^ string_of_int numTenta ^ " tentativas");
      acertar secreto numX numY (numTenta + 1)
    end
    
let rec jogar numX numY =
  let secreto = numX + Random.int numY in
  acertar secreto numX numY 0;
